...
app.post('http://localhost:8000/login', function(req, res) {
    var user_id = req.body.id;
    var email = req.body.email;
    var token = req.body.token;

    res.send(token + ' ' + user_id + ' ' + email);
    if (res) {
...
myRouter.route('/data')
    ...
    .put(function(req, res) {
        var input = "{\"email\":\"admin@admin.com\"}";


        let sqlverify = "SELECT email FROM utilisateur WHERE email = 'admin@admin.com' AND password = '" + req.body.passwordAdmin + "';";
        let query = connexion.query(sqlverify, (err, results2) => {
            if (err) throw err;
